...
- **输入方式：仅粘贴文本**：除逐字模拟键入（默认，禁止粘贴的输入框也能用）外，
  新增「仅粘贴文本」——把纯文本写回剪贴板（剥离富文本格式）后按一次
  <kbd>⌘V</kbd>。长文本瞬间完成且不带格式，但仅在允许粘贴的输入框有效。
  菜单栏与设置中均可切换，对主热键和历史条目同样生效；CLI 对应
  `--action type|paste`。
- **剪贴板历史：快捷键 + 数字**：开启历史后，用「修饰键 + 数字键 1–9、0」
  （默认 <kbd>⌃⇧</kbd>+数字）直接输入对应的历史记录，无需打开任何界面。
  默认保留 20 条（可选 20/50/100）；菜单栏子菜单点选条目即直接输入
  （此前仅放回剪贴板）。
- **置顶浮窗**：可开关的常驻浮窗，随时展示最近的复制记录（展示数量
  5/10/15/20 可设），点击条目即输入；浮窗不抢焦点，在所有桌面空间保持可见，
  支持拖动、单条删除与清空。
- **快捷键自定义**：全部快捷键（键入剪贴板 / 历史数字组合 / 浮窗开关）改为
  录制式自定义，点击输入框后按下想要的组合即可；替换原先"四选一"的预设，
  旧版本的预设选择会自动迁移。注册失败（组合被其他应用占用）时在设置中提示。
...
- CLI：`--stdin`，从标准输入读取要输入的文本，代替剪贴板（供应用直接键入历史
  条目，内容不进入进程参数）。
- CLI：`--action type|paste`，选择键入或「仅粘贴文本」方式，单次 / 热键 /
  托盘三种模式均支持。
